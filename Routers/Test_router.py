...
async def get_purchase_price_mean(db: Session = Depends(SessionLocal)):
...
async def get_shipments_count(per_page: int = 10, page: int = 0, db: Session = Depends(SessionLocal)):
    """Get shipments count"""
...
async def get_join(per_page: int = 10, page: int = 0, db: Session = Depends(SessionLocal)):
...
async def get_products_with_volume(volume: int, per_page: int = 10,page: int = 0, db: Session = Depends(SessionLocal)):
...
async def get_grouped_sorted_by_unit(per_page: int = 10, page: int = 0, db: Session = Depends(SessionLocal)):
...
async def update_sale_price(new_price: float, exp_date: datetime, db: Session = Depends(SessionLocal)):
...
async def generate_products_by_amount(amount: int, db: Session = Depends(SessionLocal)):
...
async def generate_enterprises_by_amount(amount: int, db: Session = Depends(SessionLocal)):
...
async def generate_shipments_by_amount(quantity: int, product_id_start: int, product_id_end: int,
                                       enterprise_id_start: int, enterprise_id_end: int,
